...
nuc_diversity <- function(window_size=NA,sliding = 0,fas_data_path = NA, one_side_length ){
...
####################local recombination################################
nuc_diversity(window_size = 10000,one_side_length=50000,fas_data_path = "C:/Users/lingo1st/Dropbox/林冠瑜/gbs_dataset_result/local_hotspot_fasta" ,sliding = 0)
local_plot_mat <- as.data.frame(plot_mat) 
local_recomb_value <- apply(local_plot_mat,1,function(x){
  mean(x)
})
local_recomb_value
