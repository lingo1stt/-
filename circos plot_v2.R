...
data <- read.table("C:/Users/lingo1st/Dropbox/林冠瑜/gbs_dataset_result/local_recomb/gbs_local_recomb.txt",header = T)
data$spline[which(data$spline <=0)] <- 0
...
y <-  rnorm(n = 6980, mean = 20, sd = 3)
recomb_df <- data.frame(y = y,x = breakpoint$Pos,chr = breakpoint$Chr)
recomb_df$chr <- paste0("chr",recomb_df$chr)
...
colnames(local_hotspot)[colnames(local_hotspot) == "map"] <- "chr"
...
poisson_hotspot$chr <- paste0("chr",poisson_hotspot$chr)
##centromere region
...
for (i in 1:nrow(local_hotspot)) {
  circos.rect(local_hotspot$start[i], -20 ,local_hotspot$end[i], 70,col = "red",lwd = 2,sector.index = local_hotspot$chr[i],border = NA)
}
### 加入centromere 區域
for(i in 1:12){
  circos.rect(cent_min[i], -20 ,cent_max[i], 70,col = "azure3",lwd = 2,sector.index = paste0("chr",i),border = NA)
...
for (i in 1:nrow(poisson_hotspot)) {
  circos.rect(poisson_hotspot$start[i], 10 ,poisson_hotspot$end[i], 30,col = "red",lwd = 3,sector.index = poisson_hotspot$chr[i],border = NA)
}
##
